...
export const RecognizedElementsView: React.FC<RecognizedElementsViewProps> = ({
  ...
  const presetName = presetId.charAt(0).toUpperCase() + presetId.slice(1).replace("-", "-");
    ...
    <ScrollArea className="h-full">
      <div className="p-4 space-y-6">
        ...
        <div className="text-sm text-muted-foreground">
          These are the recognized element terms for the {presetName} preset. Using these terms in your class names will
          pass validation.
        </div>
        ...
        {Object.entries(elements).map(([category, terms]) => (
          <div key={category} className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2">
              {terms.map((term) => (
                <Badge key={term} variant="secondary" className="justify-start font-mono text-xs">
                  {term}
                </Badge>
              ))}
            </div>
        ...
        {projectElements.length > 0 && (
          <div className="space-y-2">
            <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Project-Defined</h3>
            <div className="grid grid-cols-2 gap-2">
        ...
        {/* Usage Examples */}
        <div className="space-y-2">
          <h3 className="text-xs font-medium text-muted-foreground uppercase tracking-wide">Example Usage</h3>
          <div className="space-y-1 text-xs text-muted-foreground">
            {examples.map((example) => (
              <div key={example.code}>
                <code className="bg-muted px-1 py-0.5 rounded">{example.code}</code> - {example.description}
              </div>
            ))}
          </div>
        </div>
