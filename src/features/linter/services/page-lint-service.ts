...
export function createPageLintService(deps: { contextService: LintContextService; ruleRunner: RuleRunner }) {
  ...
  async function lintCurrentPage(elements: WebflowElement[]): Promise<RuleResult[]> {
    ...
    const allAppliedStyles: StyleWithElement[] = Array.from(context.elementStyleMap.values()).flat();
